...
const WiFiPublicDashboard = (props) => {
    ...
    const [wifi, setWifiName] = useState(ipcRenderer.sendSync('getWifiName', '') || 'wifi')
    ...
    function getGraphData(count, date) {
        ...
        if (graphtime == 10) {
            if (counts.length) {
                ...
                // secondsStr++
                arr.push({ "x": date.slice(11), "y": sum })
            }
        }
        else if (graphtime == 100) {
            if (counts.length >= 10) {
    ...
    async function getData() {
        ...
        if (response.ok) {
            let convertedDate = new Date(respData.posts[0].RTC.replace('_', ' ') + ' UTC').toLocaleString('en-US', { hour12: false }, { timeZone: 'Asia/Kolkata' })
            setStatus('success')
            // if (pulse !== -1) {
            ...
            if (respData.posts[0].packetnumber !== packetNumber) {
                ...
                for (let i = 0; i < dummyDataPoints.length; i++) {
                    ...
                    excelDataArr.push({ device_ID, timeShorts, UTCtoISTDate, countPulse, battery })
                }
            ...
            <div className='rechart-container'>
                <ResponsiveContainer
                    ...
                    >
                        ...
                        <Tooltip />
                        <Line strokeWidth={2} type="monotone" isAnimationActive={false} dataKey="count" stroke="#00008B" />
                    </LineChart>
                </ResponsiveContainer>
